...
export default function AdminProfilePage() {
  ...
  const handleSaveImage = async () => {
    ...
    setIsLoading(true);
    try {
      const updated = await updateProfile({photoURL: finalImage});
      setUser(updated);
  ...
  const fallbackAvatar = (n: string) =>
    `https://ui-avatars.com/api/?name=${encodeURIComponent(n)}&background=ef4444&color=fff&size=96`;
  ...
  return (
    <div className="p-6 pt-8 max-w-4xl mx-auto space-y-6">
      <motion.div initial={{opacity: 0, y: 20}} animate={{opacity: 1, y: 0}}>
        ...
        <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Manage your admin information and photo</p>
      </motion.div>
              ...
              {isEditingName ? (
                <div className="flex items-center gap-2 flex-1">
                  <input
                    ...
                    className="flex-1 px-3 py-1.5 text-lg font-bold bg-green-50 dark:bg-green-900/20 border border-green-300 dark:border-green-700 rounded-xl text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-green-500/40"
                    autoFocus
                  />
                  <button
                    onClick={handleSaveName}
                    disabled={isLoading}
                    className="p-1.5 rounded-lg bg-green-500 text-white hover:bg-green-400 transition-colors disabled:opacity-50"
                  >
      ...
      >
        {[
          {icon: User, label: "Full Name", value: name, color: "text-green-500", bg: "bg-green-50 dark:bg-green-900/20"},
          {icon: Mail, label: "Email Address", value: user.email, color: "text-emerald-500", bg: "bg-emerald-50 dark:bg-emerald-900/20"},
          {icon: Shield, label: "Account Role", value: "🛡️ Admin", color: "text-teal-500", bg: "bg-teal-50 dark:bg-teal-900/20"},

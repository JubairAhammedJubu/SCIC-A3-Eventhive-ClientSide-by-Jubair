...
export default function LoginPage() {
  ...
  const handleDemoLogin = async (role: "user" | "admin") => {
    const creds =
      role === "admin"
        ? {email: "Jubair34@gmail.com", password: "Jubair34"}
        : {email: "demo@eventhive.app", password: "Demo@123"};
    setFormData(creds);
    setDemoLoading(role);
    try {
  ...
  return (
    <div className="flex-1 flex items-center justify-center px-4 py-12 min-h-screen bg-gray-50 dark:bg-[#0f1117] relative overflow-hidden">
      ...
      >
        <div className="bg-white/80 dark:bg-[#1a1d24]/80 backdrop-blur-xl border border-gray-200 dark:border-gray-800 rounded-3xl overflow-hidden shadow-2xl hover:shadow-indigo-500/10 dark:hover:shadow-indigo-900/30 transition-all duration-300 flex flex-col md:flex-row">
          {/* Image Panel */}
          <div className="relative w-full md:w-1/2 h-48 md:h-auto overflow-hidden md:order-2">
            ...
            <div className="absolute inset-0 flex flex-col justify-end p-6 md:p-8">
              <span className="bg-gradient-to-r from-indigo-400 to-violet-400 bg-clip-text text-transparent font-bold text-2xl md:text-3xl drop-shadow-sm mb-3">
                EventHive
              </span>
              <p className="text-white/90 text-sm md:text-base leading-relaxed max-w-[240px]">
                Find and book events, or pick up right where you left off hosting your own.
              </p>
            </div>
          </div>
          ...
          <div className="w-full md:w-1/2 p-8 md:order-1">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-500 dark:text-gray-400 text-sm">
                Log in to book and manage events.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 my-4">
              <div className="flex-1 h-px bg-gray-200 dark:bg-gray-700" />
              <span className="text-gray-400 dark:text-gray-500 text-xs">Or try a demo account</span>
              <div className="flex-1 h-px bg-gray-200 dark:bg-gray-700" />
            </div>
            ...
            <div className="flex gap-2">
              <motion.button
                ...
                whileTap={{scale: 0.95}}
                className="flex-1 py-2.5 rounded-xl bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700 hover:border-gray-300 dark:hover:border-gray-600 text-gray-900 dark:text-white text-xs font-medium flex items-center justify-center gap-2 transition-all duration-200 disabled:opacity-60 disabled:cursor-not-allowed"
              >
                {demoLoading === "user" ? (
